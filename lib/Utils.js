...
class Utils {
  ...
  static sign({ apiMethod, apiPath, body, params, secret, timestamp }) {
    // let auth = timestamp + apiMethod + apiPath;
    // if ((apiMethod === 'POST') || (apiMethod === 'DELETE')) {
    //   auth += bodyString;
    // }
    ...
    switch (method) {
      ...
      case 'DELETE':
        sign += `/api/${apiPath}?/`
        // sign += `/api/${apiPath}?${new URLSearchParams(params).toString()}`
        break;
      case 'POST':
      case 'PUT':
      case 'PATCH':
    ...
    return hmacSHA256(sign, secret).toString(Hex)
  }
